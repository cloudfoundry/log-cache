...
var _ = Describe("CfAuthMiddleware", func() {
	...
	Describe("/v1/shard_promql", func() {
		BeforeEach(func() {
			spyOauth2ClientReader.client = "some-client-id"
			spyOauth2ClientReader.user = "some-user-id"

			request = httptest.NewRequest(http.MethodGet, "/v1/shard_promql/some-name", nil)
		})

		Describe("Add to group", func() {
			BeforeEach(func() {
				spyPromQLParser.sourceIDs = []string{"some-id"}
				request.URL.Path = "/v1/shard_promql/some-name"
				request.Method = "PUT"
				request.Body = ioutil.NopCloser(strings.NewReader(`{"query":"metric"}`))
			})
			...
			It("prefixes group name for GET request with the client_id and user_id", func() {
				...
				Expect(req.URL.Path).To(Equal("/v1/shard_promql/some-client-id-some-user-id-some-name"))
				Expect(spyOauth2ClientReader.token).To(Equal("valid-token"))

				Expect(reqBody).To(MatchJSON(`{"query":"metric"}`))
			...
			It("returns 404 if user is not authorized", func() {
				...
				Expect(recorder.Code).To(Equal(http.StatusNotFound))
				Expect(baseHandlerCalled).To(BeFalse())
				Expect(spyPromQLParser.query).To(Equal(`metric`))
			})

			It("returns 400 if query does not parse", func() {
		...
		Describe("Read from promQL group", func() {
			BeforeEach(func() {
				request.URL.Path = "/v1/shard_promql/some-name"
				request.Method = "GET"
			})
		...
		Describe("Read meta for promql sharding", func() {
			BeforeEach(func() {

				request.URL.Path = "/v1/shard_promql/some-name/meta"
				request.Method = "GET"
			})
			...
			It("prefixes group name for GET request with the client_id and user_id", func() {
				...
				Expect(req.URL.Path).To(Equal("/v1/shard_promql/some-client-id-some-user-id-some-name/meta"))
				Expect(spyOauth2ClientReader.token).To(Equal("valid-token"))
			})
