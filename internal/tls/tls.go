...
func (t TLS) ExperimentalCredentials(cn string) credentials.TransportCredentials {
	creds, err := NewTLSExperimentalCredentials(t.CAPath, t.CertPath, t.KeyPath, cn)
	if err != nil {
		log.Fatalf("failed to load TLS config: %s", err)
	}
...
func NewTLSExperimentalCredentials(
	caPath string,
	certPath string,
	keyPath string,
	cn string,
) (credentials.TransportCredentials, error) {
	cfg, err := NewTLSExperimentalConfig(caPath, certPath, keyPath, cn)
	if err != nil {
		return nil, err
	}
...
func NewTLSExperimentalConfig(caPath, certPath, keyPath, cn string) (*tls.Config, error) {
