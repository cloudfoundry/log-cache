...
var _ = Describe("PromQL", func() {
	...
	Context("When using a RangeQuery", func() {
		...
		It("returns a matrix of aggregated values when given a float step", func() {
			lastHour := time.Now().Truncate(time.Hour).Add(-time.Hour)
			...
			spyDataReader.readResults = [][]*loggregator_v2.Envelope{
				{
					...
					{
						SourceId:  "some-id-1",
						Timestamp: lastHour.Add(1500 * time.Millisecond).UnixNano(),
						Message: &loggregator_v2.Envelope_Gauge{
							Gauge: &loggregator_v2.Gauge{
								Metrics: map[string]*loggregator_v2.GaugeValue{
									"metric": {Unit: "thing", Value: 105},
					...
					{
						SourceId:  "some-id-1",
						Timestamp: lastHour.Add(3 * time.Second).UnixNano(),
						Message: &loggregator_v2.Envelope_Gauge{
							Gauge: &loggregator_v2.Gauge{
								Metrics: map[string]*loggregator_v2.GaugeValue{
									"metric": {Unit: "thing", Value: 107},
			...
			r, err := q.RangeQuery(
				context.Background(),
				&logcache_v1.PromQL_RangeQueryRequest{
					Query: `avg_over_time(metric{source_id="some-id-1"}[1s])`,
					Start: lastHour.UnixNano(),
					End:   lastHour.Add(3 * time.Second).UnixNano(),
					Step:  "1.5",
				},
			)
			...
			Expect(r.GetMatrix().GetSeries()).To(Equal([]*logcache_v1.PromQL_Series{
				{
					Metric: map[string]string{},
					Points: []*logcache_v1.PromQL_Point{
						{Time: lastHour.UnixNano(), Value: 99},
						{Time: lastHour.Add(1500 * time.Millisecond).UnixNano(), Value: 105},
						{Time: lastHour.Add(3 * time.Second).UnixNano(), Value: 107},
					},
				},
			}))
		})
		...
		DescribeTable("it supports all Prometheus-accepted units",
			func(unit string, step time.Duration) {
				lastHour := time.Now().Truncate(time.Hour).Add(-time.Hour)
				var times []time.Time

				for i := 1; i < 4; i++ {
					times = append(times, lastHour.Add(time.Duration(i)*step))
				}

				var readResult []*loggregator_v2.Envelope

				for i, time := range times {
					readResult = append(readResult, &loggregator_v2.Envelope{
						SourceId:  "some-id-1",
						Timestamp: time.UnixNano(),
						Message: &loggregator_v2.Envelope_Gauge{
							Gauge: &loggregator_v2.Gauge{
								Metrics: map[string]*loggregator_v2.GaugeValue{
									"metric": {Unit: "thing", Value: float64(100 + i)},
								},
							},
						},
						Tags: map[string]string{},
				...
				spyDataReader.readErrs = []error{nil}
				spyDataReader.readResults = [][]*loggregator_v2.Envelope{readResult}
				...
				r, err := q.RangeQuery(
					context.Background(),
					&logcache_v1.PromQL_RangeQueryRequest{
						Query: `avg_over_time(metric{source_id="some-id-1"}[1` + aggregationUnit + `])`,
						Start: lastHour.UnixNano(),
						End:   lastHour.Add(time.Duration(3) * step).UnixNano(),
						Step:  "1" + unit,
					},
				)
				...
				Expect(r.GetMatrix().GetSeries()).To(Equal([]*logcache_v1.PromQL_Series{
					{
						Metric: map[string]string{},
						Points: []*logcache_v1.PromQL_Point{
							{Time: times[0].UnixNano(), Value: 100},
							{Time: times[1].UnixNano(), Value: 100.5},
							{Time: times[2].UnixNano(), Value: 101.5},
						},
					},
				}))
			},
			Entry("unlabeled second", "", time.Second),
			Entry("second", "s", time.Second),
			Entry("minute", "m", time.Minute),
			Entry("hour", "h", time.Hour),
			Entry("day", "d", 24*time.Hour),
			Entry("week", "w", 7*24*time.Hour),
			Entry("year", "y", 365*24*time.Hour),
		)
